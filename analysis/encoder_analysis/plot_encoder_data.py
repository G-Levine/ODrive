...
with open(FILENAME) as csv_file:
    ...
    data_counts = [int(float(x[0])) // (2 ** BITS_PER_COUNT) for x in list(csv_reader) if len(x)]

sd_counts = statistics.stdev(data_counts)
mean_counts = statistics.mean(data_counts)

data_counts_filtered = list(filter(lambda x: mean_counts - 3 * sd_counts < x < mean_counts + 3 * sd_counts, data_counts))
...
plt.subplot(1, 3, 2)
plt.hist(data_counts_filtered, bins=max(data_counts_filtered) - min(data_counts_filtered))
